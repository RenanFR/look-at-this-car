...
class LicensePlateServiceUnitTest {
    ...
    fun shouldGetCarInformationFromLicensePlate() {
        ...
        every {
            placaAPIClient.getBrazilCarInformationFromLicensePlate(
                ...
                vehicle
        val vehicleEntity = getSampleVehicleRecord()
        every { vehicleRepository.save(any()) } returns vehicleEntity
        every { vehicleEnquiryRepository.save(any()) } returns VehicleEnquiry(
            null,
            LocalDateTime.now(),
            "RTN8G99",
            vehicleEntity
    ...
    companion object {
        fun getSampleVehicleRecord(
        ): VehicleEntity {
            val vehicleMapper = Mappers.getMapper(VehicleMapper::class.java)
            return vehicleMapper.entityFromDataDTO(getSampleVehicle().vehicleData)
        }

        fun getSampleVehicle() = Vehicle(
